...
function buildTree(files, offsets) {
  ...
  files.forEach((file, index) => {
    ...
    const normalizedPath = `${parts.dir}/${parts.name}.${parts.ext}`.toLowerCase();
    if (groups.has(normalizedPath)) {
      throw new Error(`Duplicate VPK path: ${file.path}`);
    }
    groups.set(normalizedPath, { ...parts, index, bytes: file.bytes });
  });

  const byExt = new Map();
  for (const entry of groups.values()) {
    if (!byExt.has(entry.ext)) byExt.set(entry.ext, new Map());
    const extGroup = byExt.get(entry.ext);
    if (!extGroup.has(entry.dir)) extGroup.set(entry.dir, []);
    extGroup.get(entry.dir).push(entry);
  }

  const out = [];
  [...byExt.keys()].sort().forEach((ext) => {
    pushCString(out, ext);
...
export function writeVpk(inputFiles) {
  ...
  const totalSize = HEADER_SIZE + tree.byteLength + offset;
